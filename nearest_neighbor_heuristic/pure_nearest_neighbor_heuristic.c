...
static struct PointArray sort_by_nearest_neighbors(int *ret, int *points, int amt_points) { /* {{{ */
    ...
    memmove(/*dest=*/sorted_points, /*src=*/ret, /*n=*/(size_t)amt_points); // NOLINT(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
    int *P[amt_points];
    ...
    while (unvisited_points(/*points=*/P, amt_points)) {
        ...
        int closest_point_idx = closest_unvisited_point_idx(
            /*visited_point=*/sorted_points[i - 1], P,
            /*num_points=*/amt_points);
        sorted_points[i] = *P[closest_point_idx];
    ...
    memmove(/*dest=*/(void *)parray.points, /*src=*/sorted_points, /*n=*/(size_t)amt_points * sizeof(int)); // NOLINT(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
    #pragma clang diagnostic pop
    return parray;
}
/* }}} */
static void test_GivenPointsNotSortedByNearestNeighbors_SortByNearestNeighbors(void) {
    ...
    TEST_ASSERT_EQUAL_INT_ARRAY(/*expected=*/expected, /*actual=*/sort_by_nearest_neighbors(/*ret=*/result, points, /*amt_points=*/sizeof(expected) / sizeof(int)).points, /*num_elements=*/sizeof(expected) / sizeof(int));
    TEST_ASSERT_FALSE(memcmp(/*s1=*/expected, /*s2=*/result, /*n=*/sizeof(expected)) == 0);
}
